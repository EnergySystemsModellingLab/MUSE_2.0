    ...
    I: Iterator<Item = AgentSearchSpaceRaw>,
{
    ...
    // for (agent_id,search_space_map )in search_spaces.iter() {
    //     let missing_years =
...
mod tests {
    ...
    #[rstest]
    fn test_search_space_raw_into_search_space_invalid_missing_year(
        agents: AgentMap,
        process_ids: IndexSet<ProcessID>,
        commodity_ids: HashSet<CommodityID>,
    ) {
        ...
        assert_error!(
            raw.into_agent_search_space(&agents, &process_ids, &commodity_ids, &[2020, 2030]),
            "" // ??
        );
    }
}
