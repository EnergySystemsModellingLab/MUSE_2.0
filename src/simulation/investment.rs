...
fn select_best_assets(
    ...
    while is_remaining_unmet_demand(&demand) {
        ...
        for asset in opt_assets.iter() {
            ...
            if current_best.as_ref().is_none_or(|best_output| {
                best_output.tool_output.comparison_metric() < output.tool_output.comparison_metric()
            }) {
        ...
        let best_asset = best_output.asset;
        let tool_output = &best_output.tool_output;
        demand = tool_output.into_unmet_demand(&best_asset, commodity_id, demand);
