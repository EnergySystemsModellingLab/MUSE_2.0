...
fn get_candidate_assets<'a>(
    ...
    producers.map(move |process| {
        let asset = Asset::new_without_capacity(
            Some(agent.id.clone()),
            process.clone(),
            region_id.clone(),
            year,
        ...
        let capacity = get_demand_limiting_capacity(time_slice_info, &asset, commodity_id, demand);
        asset.with_capacity(capacity).into()
    })
}
...
fn select_best_assets(
    ...
    while is_remaining_unmet_demand(&demand) {
        ...
        for asset in opt_assets.iter() {
            let asset = if asset.is_commissioned() {
                asset.clone()
            } else {
                ...
                let capacity = max_capacity.min(remaining_capacity);
                Asset::clone(asset).with_capacity(capacity).into()
            };
