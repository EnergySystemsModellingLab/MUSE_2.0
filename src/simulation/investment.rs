...
fn select_best_assets(
    ...
    while is_any_remaining_demand(&demand) {
        ...
        for asset in opt_assets.iter() {
            ...
            // Store the appraisal results if the capacity is positive, otherwise we won't consider
            // the asset for investment. This can happen if the asset has zero activity limits for
            // all time slices with demand.
